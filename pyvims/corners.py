...
def cube_paths(cube, corners=True, lon_e=False):
    """Extract all pixels paths from a cube.
    ...
    corners: bool, optional
        Use corners for the footprint on the ground if ``TRUE`` (default).
        Otherwise, use pixel octagonal footprint.
    lon_e: bool, optional
        Use west longitude [0°, 360°[ if ``FALSE`` (default).
        Other use east longitude ]-180°, 180°].

    Returns
    -------
    matplotlib.path.Path
        Global path of the FOV.

    """
    lons = []
    lats = []
    codes = []
    for l in range(1, cube.NL + 1):
        for s in range(1, cube.NS + 1):
            pix = cube[s, l]
            footprint = pix.corners if corners else pix.footprint

            if footprint.ground:
                path = footprint.path_180 if lon_e else footprint.path_360
                lon, lat = np.transpose(path.vertices)
                lons += list(lon)
                lats += list(lat)
                codes += list(path.codes)

    return Path(np.vstack([lons, lats]).T, codes)
