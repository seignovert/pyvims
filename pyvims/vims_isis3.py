...
def pvl_fix(values):
    """Fix PVL error in ISIS3 array import.

    See Also
    --------
    https://github.com/planetarypy/pvl/pull/34

    """
    return [float(v[:-1]) if isinstance(v, str) else v for v in values]


def pvl_dt(time):
    """Convert PVL string time to DateTime."""
    return time if isinstance(time, dt) else dt.strptime(time, '%Y-%jT%H:%M:%S.%f')
...
class VIMS_ISIS3(VIMS_OBJ):
    ...
    def readLBL(self):
        ...
        try:
            ...
            wvlns_IR = pvl_fix(self.lbl['BandBin']['Center'])
            ...
            try:
                ...
                wvlns_VIS = pvl_fix(lbl_vis['BandBin']['Center'])
            ...
            try:
                ...
                wvlns_VIS = pvl_fix(self.lbl['BandBin']['Center'])
        ...
        self.start = pvl_dt(self.lbl['Instrument']['StartTime'])
        self.stop = pvl_dt(self.lbl['Instrument']['StopTime'])
