...
def slon(lon, precision=0):
    ...
    if abs(deg180(lon_e - 180)) <= 1.0e-2:
        return '180°'
...
def slon_w(lon_w, precision=0):
    ...
    """
    return '0°' if abs(lon_w) <= 1.0e-2 else f'{lon_w:.{precision}f}°W'
...
def slon_e(lon_e, precision=0):
    ...
    """
    return (
        f'{abs(lon_e):.{precision}f}°'
        f'{"" if abs(lon_e % 180) <= 1.0e-2 else "E" if lon_e > 0 else "W"}'
    )
