...
from pyvims.projections import Orthographic, Path3D
...
def test_ortho_npole_path_1(npole):
    ...
    assert_array(path.codes, [Path.MOVETO] + 7 * [Path.LINETO] + [Path.CLOSEPOLY])
...
def test_ortho_npole_path_2(npole):
    ...
    assert_array(path.codes, [Path.MOVETO] + 12 * [Path.LINETO] + [Path.CLOSEPOLY])
...
def test_ortho_spole_path_1(spole):
    ...
    assert_array(path.codes, [Path.MOVETO] + 8 * [Path.LINETO] + [Path.CLOSEPOLY])
...
def test_ortho_spole_path_2(spole):
    ...
    assert_array(path.codes, [Path.MOVETO] + 11 * [Path.LINETO] + [Path.CLOSEPOLY])
...
def test_ortho_spole_path_reverse(spole):
    ...
    assert_array(path.codes, [Path.MOVETO] + 10 * [Path.LINETO] + [Path.CLOSEPOLY])
