...
def test_stere_xy(proj):
    ...
    assert_array(proj([0, 90], [80, 80]), ([0, -450519], [-450519, 0]), decimal=0)

    assert_array(
        proj([[0], [90]], [[80], [80]]), ([[0], [-450519]], [[-450519], [0]]), decimal=0
    )
...
def test_stere_lonlat(proj):
    ...
    assert_array(
        proj([-450519, 450519], 0, invert=True), ([90, 270], [80, 80]), decimal=0
    )

    assert_array(
        proj([0, -450519], [-450519, 0], invert=True), ([0, 90], [80, 80]), decimal=0
    )
...
def test_stere_patch(proj):
    ...
    assert patch.get_facecolor() == (1, 0, 0, 0.5)
    assert patch.get_edgecolor() == (0, 0, 1, 0.5)
...
def test_stere_collection(proj):
    ...
    assert_array(coll.get_facecolors()[0], (1, 0, 0, 0.5))
    assert_array(coll.get_edgecolors()[0], (0, 0, 1, 0.5))
