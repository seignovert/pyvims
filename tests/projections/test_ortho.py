...
def test_ortho_xy(proj):
    ...
    assert_array(
        proj([0, 90, 180, -90], 0), ([0, -1, np.nan, 1], [0, 0, np.nan, 0]), decimal=1
    )

    assert_array(proj(0, [0, 90, -90]), ([0, 0, 0], [0, 1, -1]), decimal=1)

    assert_array(proj([0, 90], [90, 0]), ([0, -1], [1, 0]), decimal=1)
...
def test_ortho_xy_alt(proj):
    ...
    assert_array(
        proj([0, 90, 180, -90], 0, 0), ([0, -1, np.nan, 1], [0, 0, np.nan, 0]), decimal=1
    )
    ...
    assert_array(proj(0, [0, 90, -90], 0), ([0, 0, 0], [0, 1, -1]), decimal=1)

    assert_array(proj(90, 0, [0, 1e-3, 2e-3]), ([-1, -2, -3], [0, 0, 0]), decimal=1)

    assert_array(proj([0, 90], [90, 0], 0), ([0, -1], [1, 0]), decimal=1)

    assert_array(proj([0, 90], [90, 0], 1e-3), ([0, -2], [2, 0]), decimal=1)

    assert_array(proj([0, 90], [90, 0], [1e-3, 2e-3]), ([0, -3], [2, 0]), decimal=1)

    assert_array(proj([[0], [90]], [[90], [0]], 0), ([[0], [-1]], [[1], [0]]), decimal=1)

    assert_array(
        proj([[0], [90]], [[90], [0]], 1e-3), ([[0], [-2]], [[2], [0]]), decimal=1
    )
...
def test_ortho_lonlat(proj):
    ...
    assert_array(proj([-1, 1], 0, invert=True), ([90, 270], [0, 0]), decimal=1)
    ...
    assert_array(
        proj([[0], [-1]], [[1], [0]], invert=True), ([[0], [90]], [[90], [0]]), decimal=1
    )
...
def test_ortho_lonlat_alt(proj):
    ...
    assert_array(
        proj(1, 1, True, invert=True), (270, 45, (np.sqrt(2) - 1) * 1e-3), decimal=1
    )

    assert_array(
        proj([-1, 1], 0, True, invert=True), ([90, 270], [0, 0], [0, 0]), decimal=1
    )

    assert_array(
        proj([-2, 3], 0, True, invert=True), ([90, 270], [0, 0], [1e-3, 2e-3]), decimal=1
    )

    assert_array(
        proj(0, [2, -3], True, invert=True), ([0, 0], [90, -90], [1e-3, 2e-3]), decimal=1
    )
