...
from pyvims.misc.greatcircle import (
    great_circle,
    great_circle_arc,
    great_circle_lat,
    great_circle_patch,
...
def test_great_circle_arc(pt1, pt2):
    ...
    assert lon[0] == approx(pt1[0], abs=0.1)
    assert lat[0] == approx(pt1[1], abs=0.1)

    assert lon[-1] == approx(pt2[0], abs=0.1)
    assert lat[-1] == approx(pt2[1], abs=0.1)

    assert lon[5] == approx(69.6, abs=0.1)
    assert lat[5] == approx(50.8, abs=0.1)
...
def test_great_circle_lat(pt1, pt2):
    ...
    assert great_circle_lat(pt1[0], *pt1, *pt2) == approx(pt1[1], abs=0.1)
    assert great_circle_lat(pt2[0], *pt1, *pt2) == approx(pt2[1], abs=0.1)

    assert great_circle_lat(0, *pt1, *pt2) == approx(9.1, abs=0.1)
    assert_array(
        great_circle_lat([90, 180, 270], *pt1, *pt2), [51.3, -9.1, -51.3], decimal=1
    )
...
def test_great_circle(pt1, pt2):
    ...
    assert lons[0] == approx(0, abs=0.1)
    assert lats[0] == approx(9.1, abs=0.1)

    assert lons[-1] == approx(360, abs=0.1)
    assert lats[-1] == approx(9.1, abs=0.1)

    assert lons[5] == approx(200, abs=0.1)
    assert lats[5] == approx(-30, abs=0.1)

    lons, lats = great_circle(*pt1, *pt2, npt=10, lon_e=True)

    assert len(lons) == len(lats) == 10

    assert lons[0] == approx(-180, abs=0.1)
    assert lats[0] == approx(-9.1, abs=0.1)

    assert lons[-1] == approx(180, abs=0.1)
    assert lats[-1] == approx(-9.1, abs=0.1)

    assert lons[6] == approx(60, abs=0.1)
    assert lats[6] == approx(-45, abs=0.1)
...
def test_great_circle_pole_lat(pt1):
    """Test orthogonal points on the great circle from its polar axis."""
    assert great_circle_pole_lat(0, *pt1) == approx(-58.4, abs=0.1)
    assert_array(
        great_circle_pole_lat([90, 180, 270], *pt1), [-30.6, 58.4, 30.6], decimal=1
    )
...
def test_great_circle_pole(pt1):
    ...
    assert lons[0] == approx(0, abs=0.1)
    assert lats[0] == approx(-58.4, abs=0.1)

    assert lons[-1] == approx(360, abs=0.1)
    assert lats[-1] == approx(-58.4, abs=0.1)

    assert lons[5] == approx(200, abs=0.1)
    assert lats[5] == approx(60, abs=0.1)
    ...
    assert lons[4] == approx(-20, abs=0.1)
    assert lats[4] == approx(-60, abs=0.1)
...
def test_great_circle_path(pt1, pt3):
    ...
    assert_array(path.vertices[:, 1], [-58.4, 58.4, -58.4, 90.0, 90.0, -58.4], decimal=1)
...
def test_great_circle_patch(pt1):
    ...
    patch = great_circle_patch(
        *pt1, npt=3, lon_e=True, inside=False, color='orange', alpha=0.1
    )
    path = patch.get_path()
